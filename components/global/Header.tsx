...
// import { getBootcampMegaMenuData, getCertificationsMegaMenuData } from "@/lib/wordpress";

// export default async function Header() {
//   const [bootcamps, certs] = await Promise.all([
//     getBootcampMegaMenuData(),
//     getCertificationsMegaMenuData(),
//   ]);

//   return (
//     <HeaderClient
//       bootcampParents={bootcamps.bootcampCategories?.nodes ?? []}
//       certificationParents={certs.certificationsCategories?.nodes ?? []}
//     />
//   );
// }
import HeaderClient from "./HeaderClient";
...
export default async function Header() {
  const [bootcamps, certs, headerSettings] = await Promise.all([
    getBootcampMegaMenuData(),
    getCertificationsMegaMenuData(),
    getHeaderSettingsData(),
  ]);

  const settingsNode = headerSettings?.pages?.nodes?.[0] ?? null;

  const resourcesItems = normalizeHeaderDropdownItems(
    settingsNode?.headerResourcesDropdown ?? null
  );

  const aboutItems = normalizeHeaderDropdownItems(
    settingsNode?.headerAboutDropdown ?? null
  );
